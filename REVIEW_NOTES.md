...
### 25. [arch] `propwash-core/src/types.rs:314-348` — `RawSession` enum and `Session` indirection may be unnecessary
`RawSession` is a manual vtable: each variant holds a type that already `impl Unified`, and `as_unified()` just matches and returns `&dyn Unified`. If `Unified` is made rich enough (#12), `Session` could hold `Box<dyn Unified>` directly (or be generic), eliminating the enum, the `raw` field, and the `raw` feature gate entirely. Format parsers return `Session` containing their concrete type; consumers only see `&dyn Unified`. This is the natural conclusion of #10, #12, #16 — the `RawSession` enum only exists to enable the format-specific access we're trying to remove.
